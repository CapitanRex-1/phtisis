const modalWindow = post => {
  ...
  <div class="col s12">
    ...
    <div class="card-action">
        ...
        <button class="btn btn-small grey js-arhiv" data-id="${post._id}">Вернуть в базу
...
PostApi.fetch().then(backendPosts => {
  posts = backendPosts.filter(backendPosts => backendPosts.marks === false)
    document.getElementById("amount").innerHTML = `<h4 class="left-align">Всего пациентов в архиве: ${posts.length}</h4>`
    renderPosts(posts)
})
}
...
if (event.target.classList.contains('js-arhiv') || event.target.parentNode.classList.contains('js-arhiv')) {
  const decision = confirm('Вы уверены, что хотите вернуть пациента в базу?')
  ...
  if (decision){
    ...
    PostApi.update(newPost).then(post => {
      mainRender()
      M.toast({html: 'Пациент возвращен в базу.', classes: 'rounded'})
    })       
  }
}
}
